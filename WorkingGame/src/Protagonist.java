...
public class Protagonist {
	...
	private int protagonistRepaintCounter = 15  ;	//used when protagonist is stopped
...
//				collisionBox.setLocation(currentX, currentY);
				...
				//change the current frame in animation every 3 repaints (necessary when repainting so quickly to smooth the animation
				if(moveCount == 0){
...
//				collisionBox.setLocation(currentX, currentY);
	...
	public void updateLocations(){
		...
		middleRight.setLocation(currentX + PROTAGONIST_WIDTH, ( (currentY + PROTAGONIST_HEIGHT) + currentY) / 2 + 10);	//use 10 as offset to better handle collisions from side
		middleLeft.setLocation(currentX, ( (currentY + PROTAGONIST_HEIGHT) + currentY) / 2 + 10);
	}
	...
	public void stop(){
		//when the protagonist is stopped, it is still moving slightly faster than the block it is on.
		//so, incremement the protagonist's animation frame half as fast as when it moves.
		if(protagonistRepaintCounter == 0){
			incrementFrameNumber();
			protagonistRepaintCounter = 15;
		}else{
			protagonistRepaintCounter--;
		}
		...
		for(int i = 0; i < level.getPlatforms().size(); i++){
			...
			if(this.collisionBox.intersects(level.getPlatforms().get(i).getCollisionBox())){
				...
				if(collidingPlatform.getCollisionBox().contains(topRight) || collidingPlatform.getCollisionBox().contains(middleRight) ){
					...
					descending = true;	//descends so that he does not cling to edge of platform
					jump_count = 30;	//also do not let him jump when caught at edge
...
//						descending = true;
				...
				if(collidingPlatform.getCollisionBox().contains(topLeft) || collidingPlatform.getCollisionBox().contains(middleLeft) ){
...
//						descending = true;
	...
	//					System.out.println("Collision with platform type 1");
			...
			//else, he does not collide with the platform
			} else {
				noCollide++;
			}

			//if protagonist does not collide with any platforms and is not jumping, should descend.  (gravity)
